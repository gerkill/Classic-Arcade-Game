...
// Speed Multiplier: increase this value to increase difficulty
...
// Check for collision. Basic Code Source:
// https://developer.mozilla.org/en-US/docs/Games/Techniques/2D_collision_detection
Enemy.prototype.checkCollision = function() {
	"use strict";
	// Set boxes to detect collision
	var playerBox = {
		x: player.x,
		y: player.y,
		width: 50,
	...
	// Check for collisions (when playerBox intersects with enemyBox) 
	if (playerBox.x < enemyBox.x + enemyBox.width && playerBox.x + playerBox.width > enemyBox.x && playerBox.y < enemyBox.y + enemyBox.height && playerBox.height + playerBox.y > enemyBox.y) {
		// Collision detected, call collisionDetected function
		this.collisionDetected();
	}
};
// Collision detected, decrease playerLives and reset the player
...
Gem.prototype.checkCollision = function() {
	...
	// Check for collisions (player box intersects gem box)
	if (playerBox.x < gemBox.x + gemBox.width && playerBox.x + playerBox.width > gemBox.x && playerBox.y < gemBox.y + gemBox.height && playerBox.height + playerBox.y > gemBox.y) {
		// Collision detected, call collisionDetected function
		this.collisionDetected();
	}
};
// Detect gem collision and hide gem off canvas,
// Increase player score; wait 4 seconds; reset gem.
...
Heart.prototype.checkCollision = function() {
	...
	// Check for collisions (player box intersects with heart box)
	if (playerBox.x < heartBox.x + heartBox.width && playerBox.x + playerBox.width > heartBox.x && playerBox.y < heartBox.y + heartBox.height && playerBox.height + playerBox.y > heartBox.y) {
		// Collision detected, call collisionDetected function
		this.collisionDetected();
	}
};
// When heart collision is detected, hide heart off canvas
// Increase player lives; wait 30 seconds; reset heart.
...
var heart = new Heart(101 * Math.floor(Math.random() * 4) + 0, 70 + (85 * Math.floor(Math.random() * 3) + 0));
